...
class fusion_prompt_loss(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused, task):
        ...
        for idx, task_type in enumerate(task):
            ...
            if task_type == "lowlight_contrast":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            elif task_type == "lowlight_noise":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            elif task_type == "overexposure_contrast":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            elif task_type == "overexposure_noise":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            else:
        ...
        return total_loss / num_tasks, total_max_loss / num_tasks, total_color_loss / num_tasks, total_grad_loss / num_tasks,
