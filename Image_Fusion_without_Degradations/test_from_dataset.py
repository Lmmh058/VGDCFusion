...
def get_input_texts(task):
    text_map = {
        ...
        "None": ("We're dealing with the fusion of infrared and visible light images.", "We're dealing with the fusion of infrared and visible light images.")
    }
    return text_map.get(task, ("", ""))

def channel_mean(feat):
    """
    feat: (B, C, H, W)
    """
    return feat[0].mean(dim=0)

def resize_feat(feat_map, target_hw):
    """
    feat_map: (H, W)
    target_hw: (H, W)
    """
    feat_map = feat_map.unsqueeze(0).unsqueeze(0)  # (1,1,h,w)
    feat_map = F_nn.interpolate(
        feat_map,
        size=target_hw,
        mode='bilinear',
        align_corners=False
...
def main(args):
    ...
    for i in range(len(visible_path)):
        ...
        with torch.no_grad():
            ...
            # output,enc_feats = model(vi_padded, ir_padded, text_ir, text_vi)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='MSRSRGB', help='Dataset name, e.g., "Text_Test"')
    parser.add_argument('--task', type=str, default='None', choices=[
