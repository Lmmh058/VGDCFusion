...
class fusion_loss(nn.Module):
    ...
    def forward(self, image_visible, image_infrared, image_fused, max_ratio=4, ssim_vis_ratio=1, ssim_ir_ratio=1, ssim_ratio=1, color_ratio=12, text_ratio=10):
        ...
        return total_loss, loss_max, loss_color, loss_text
...
class fusion_prompt_loss(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused, task="lowlight_contrast"):
        total_loss = 0
        total_max_loss = 0
        total_color_loss = 0
        total_grad_loss = 0
        ...
        for idx in range(batch_size):
            ...
            if task == "lowlight_contrast":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            elif task == "lowlight_noise":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            elif task == "overexposure_contrast":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            elif task == "overexposure_noise":
                loss, max_loss, color_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
                                                                                    max_ratio=10, ssim_ratio=1, text_ratio=10)
            else:
        ...
        return total_loss / batch_size, total_max_loss / batch_size, total_color_loss / batch_size, total_grad_loss / batch_size,
